...
* **Entities（實體）**：現實世界中可以被獨立識別的「人、事、物」。在 MongoDB 中，一個實體通常對應一個**集合（Collection）**或一份**文件（Document）**。
  * *例子*：`使用者 (User)`、`商品 (Product)`。
* **Attributes（屬性）**：用來描述特定實體特徵的資料。在 MongoDB 中，屬性就是文件內的**欄位（Fields）**。
  * *例子*：使用者的 `Email`、商品的 `價格 (price)`。
* **Relationship（關係）**：不同實體之間在業務邏輯上的**關聯性與互動方式**。
  * *例子*：`使用者` 購買 `商品`。

### 2. 三大資料關係類型（Relationship Types）
實體之間的關聯數量（基數，Cardinality）主要分為以下三種：

#### ① One-to-One（1:1，一對一關係）
* **定義**：一個實體 A 最多只會對應到一個實體 B，反之亦然。
* **常見場景**：`使用者` 與他的 `帳戶詳細設定`。
* **MongoDB 設計指南**：通常直接使用 **內嵌文件（Embedding）**，將兩者合併放在同一份文件裡，查詢效率最高。

#### ② One-to-Many（1:N，一對多關係）
* **定義**：一個實體 A 可以對應到多個實體 B，但一個實體 B 只能對應到一個實體 A。
* **常見場景**：`電商訂單` 與 `多筆購買商品`，或是 `部落格文章` 與 `多條留言`。
* **MongoDB 設計指南**：
...
* **定義**：一個實體 A 可以對應到多個實體 B，同時一個實體 B 也可以對應到多個實體 A。
* **常見場景**：`學生` 與 `選修課程`（一個學生選多門課，一門課有多個學生）；`書籍` 與 `作者`。
* **MongoDB 設計指南**：在 MongoDB 中不需要建立 SQL 的「中間橋接表」，通常會在其中一方（或雙方）欄位中使用 **ID 陣列（Array of ObjectIds）** 來互相引用。
