...
桶模式是一種模式設計策略，它不是將每個測量值儲存為單獨的文檔，而是根據時間間隔（例如 1 小時）將多個測量值分組（桶化）到單一文件中。這對於物聯網感測器、監控系統以及任何產生高頻時間戳資料的場景都特別有用。
...
{
  "sensor_id" : "sensor_42" , 
  "timestamp" : ISODate ( "2024-01-01T10:03:00Z" ), 
  溫度：22.5 
}
```
...
```sql
使用桶式模式，60 篇閱讀材料被歸類在一個文件中：
// 文件分組，閱讀時間 1 小時
// 這樣就將 60 個文檔合併成 1 個文檔，大大減少了索引和文檔數量。


{
  "sensor_id" : "sensor_42" , 
  "interval_start" : ISODate ( "2024-01-01T10:00:00Z" ), 
  "interval_end" : ISODate ( "2024-01-01T11:00:00Z" ), 
  “計數” ：60 ， 
  「總和」：1347.0 ， 
  “測量值” ：[ 
    { "timestamp" : ISODate ( "2024-01-01T10:00:00Z" ), "temperature" : 22.4 },     
    { "timestamp" : ISODate ( "2024-01-01T10:01:00Z" ), "temperature" : 22.5 },     
    // 最多 60 筆記錄
  ]
}
```
...
它專門用來解決上一節筆記中提到的 「無邊界文件（Unbounded Documents）」 與 「大陣列導致文件超過 16MB」 的痛點。    
當你的資料分布呈現 99% 的大眾數據都很小，但有 1% 的網紅/名人數據大到爆炸（長尾效應） 時，這個模式就是你的救星。    
    
1. 為什麼需要 Outlier Pattern？（痛點分析）    
假設你正在設計一個社群平台的「用戶粉絲追蹤系統」：    
    
一般用戶（99%）：粉絲數量通常在 10 到幾百人之間。如果把粉絲的 UserID 直接內嵌（Embed）在一個陣列欄位 followers 裡，文件大小頂多幾 KB，查詢速度極快。    

網紅/超級巨星（1%）：像是頂級名人的粉絲量高達數千萬。如果同樣把粉絲內嵌在同一個文件的陣列裡，這個陣列會瞬間暴增，直接突破 MongoDB 的 16MB 單一文件限制，導致系統崩潰。    
    
如果為了這 1% 的網紅，強迫另外 99% 的普通用戶全部改用複雜的關聯表（$lookup）查詢，會嚴重犧牲整體的查詢效能。    


**總結：優缺點與必考點**    
最大優點：兼顧效能與安全。讓 99% 的常見查詢維持在最快的內嵌模式，同時徹底解決 1% 異常資料導致的 16MB 溢出崩潰問題。    
缺點：資料庫 Schema 變得比較複雜，後端程式碼必須自己寫邏輯去處理 isOutlier 的判斷與多表寫入。    
適用考點：只要題目出現「絕大多數資料很小，但有極少數資料會無限增長（如書本的評論、名人的追蹤、熱門股票的交易紀錄）」，答案選 Outlier Pattern 就對了！    
...
儲存桶模式（Bucket Pattern） 時，我們用特定的時間範圍（例如 1 小時）或數量限制，將多筆時序資料（如每分鐘的溫度）封裝至單一文件內的陣列中。這個模式在優化「讀取效能」與「減少儲存空間」上表現極佳，但天下沒有白吃的午餐，它帶來的最主要代價（Trade-off）正是寫入複雜度的顯著提升。
