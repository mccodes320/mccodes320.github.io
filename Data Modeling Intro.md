...
當計算某個值（例如，數組求和）的成本很高，且讀取頻率遠高於底層資料變化頻率時，計算模式最為有效。

當來源資料幾乎每次請求都會發生變化時

數百萬用戶的高並發讀取是計算模式的典型應用場景

緩慢且昂貴的聚合操作實際上是使用計算模式的一個有力論據，因為預先計算結果可以避免每次讀取時都執行這種代價高昂的操作。
...
## **5. Outlier Pattern 異常值模式**

專門解決 「無邊界文件（Unbounded Documents）」 與 「大陣列導致文件超過 16MB」 的痛點, 避免這些特例拖慢整體系統效能。    
